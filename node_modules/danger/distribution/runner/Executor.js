...
var Executor = /** @class */ (function () {
    ...
    Executor.prototype.handleResultsPostingToPlatform = function (originalResults, git) {
        return __awaiter(this, void 0, void 0, function () {
            var results, fails, warnings, messages, markdowns, failureCount, messageCount, dangerID, failed, hasMessages, issueURL, previousComments, _i, previousComments_1, comment, s, are, mergedResults, previousComments, inline, inlineLeftovers, commitID, comment;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                    ...
                    case 8:
                        ...
                        mergedResults = DangerResults_1.regularResults(results);
                    ...
                    case 10:
                        ...
                        mergedResults = DangerResults_1.sortResults(DangerResults_1.mergeResults(mergedResults, inlineLeftovers));
                        _a.label = 11;
                    case 11:
                    ...
                    case 12:
                        ...
                        if (process.env["DANGER_BITBUCKETSERVER_HOST"]) {
                            comment = bitbucketServerTemplate_1.template(dangerID, mergedResults, commitID);
                        }
                        else if (process.env["DANGER_BITBUCKETCLOUD_OAUTH_KEY"] || process.env["DANGER_BITBUCKETCLOUD_USERNAME"]) {
                            comment = bitbucketCloudTemplate_1.template(dangerID, mergedResults, commitID);
                        }
                        else {
                            comment = githubIssueTemplate_1.template(dangerID, mergedResults, commitID);
                        }
    ...
    Executor.prototype.inlineCommentTemplate = function (inlineResults) {
        ...
        var comment;
        if (process.env["DANGER_BITBUCKETSERVER_HOST"]) {
            comment = bitbucketServerTemplate_1.inlineTemplate(this.options.dangerID, results, inlineResults.file, inlineResults.line);
        }
        else if (process.env["DANGER_BITBUCKETCLOUD_OAUTH_KEY"] || process.env["DANGER_BITBUCKETCLOUD_USERNAME"]) {
            comment = bitbucketCloudTemplate_1.inlineTemplate(this.options.dangerID, results, inlineResults.file, inlineResults.line);
        }
        else {
            comment = githubIssueTemplate_1.inlineTemplate(this.options.dangerID, results, inlineResults.file, inlineResults.line);
        }
        return comment;
    };
    /**
...
var messageForResults = function (results) {
    if (!results.fails.length && !results.warnings.length) {
        return "All green. " + DangerUtils_1.compliment();
    }
    else {
        if (process.env["DANGER_BITBUCKETSERVER_HOST"]) {
